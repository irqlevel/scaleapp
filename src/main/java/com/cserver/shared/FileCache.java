...
class FileCacheClearTask extends TimerTask {
	...
	public void run() {
		...
		SLog.i(TAG, "task running");
		cache.clear();
		SLog.i(TAG, "task completed");
	}
}
...
public class FileCache {
	...
	File getFileDir(String resourceId) {
		...
		if (urlSum == null) {
			SLog.e(TAG, "md5 returns null for " + resourceId);
			return null;
		}
	...
	public File getFile(String resourceId) {
        ...
        if (!file.exists()) {
        	SLog.e(TAG, "file is not exists " + file.getAbsolutePath());
        	return null;
        }
	...
	private void clearDirByLevel(File dir, int level) {
		if (level >= 4) {
			...
			try {
				if (mapFile.exists()) {
					SLog.i(TAG, "found map file here " + mapFile.getAbsolutePath());
			        ...
			        for (String key: map.keySet()) {
			        	...
			        	if ((current > lastModified) && ((current - lastModified) > clearTaskFileOldTime)) {
			        		SLog.i(TAG, "file=" + file.getAbsolutePath() + " selected for delete");
			        		keys2Delete.add(key);
			        	}
			        }
					...
					SLog.i(TAG, "map file not found will delete dir=" + dir.getAbsolutePath());
					FileOps.deleteFileRecursive(dir);
				}
			} catch (Exception e) {
				SLog.exception(TAG, e);
