...
public class MessageCrypt {	
	...
	public static MessageCryptResult encryptMessage(String privKeyS, long privKeyId, Message msg, MessageCryptResolver resolver, IRealClock clock) {	
		...
		if (debug) {
			SLog.d(TAG, "encryptMessage:privKeyS=" + privKeyS);
			SLog.d(TAG, "encryptMessage:privKeyId=" + privKeyId);
			SLog.d(TAG, "encryptMessage:msg.id=" + msg.id);
			SLog.d(TAG, "encryptMessage:msg.from=" + msg.from);
			SLog.d(TAG, "encryptMessage:msg.peer=" + msg.peer);	
		}
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:getUserCurrentPubKey time=" + clock.elapsedTime());
		
		if (queryResult.error != Errors.SUCCESS) {
			if (debug)
				SLog.e(TAG, "encryptMessage:getCurrentPubKey error= " + queryResult.error);
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:Json.stringToPublicKey time=" + clock.elapsedTime());
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:Json.stringToPrivateKey time=" + clock.elapsedTime());
		
		msg.encKeyId = queryResult.keyId;
		msg.signKeyId = privKeyId;
		if (debug) {
			SLog.d(TAG, "encryptMessage:msg.encKeyId=" + msg.encKeyId);
			SLog.d(TAG, "encryptMessage:msg.signKeyId=" + msg.signKeyId);
			SLog.d(TAG, "encryptMessage:pubKey=" + JsonHelper.publicKeyToString(dstPubKey));
		}
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:gen random AES key time=" + clock.elapsedTime());
		
		if (debug) {
			SLog.d(TAG, "encryptMessage:aesKey=" + Base64.encode(aesKey));
		}
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:rsaEncrypt AES key time=" + clock.elapsedTime());
		
		if (encryptedAesKey == null) {
			if (debug)
				SLog.e(TAG, "encryptMessage:encryptedAesKey is null");
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:msg.toBytes() time=" + clock.elapsedTime());
		
		if (clock != null)
			clock.start();
        byte bodyHash[] = sha256(body);
		if (clock != null)
			SLog.d(TAG, "encryptMessage:sha256(body time=" + clock.elapsedTime());
		
        if (bodyHash == null) {
        	if (debug)
        		SLog.e(TAG, "encryptMessage:bodyHash is null");
			result.error = Errors.MSG_BODY_HASH_FAILED;
        	return result;
        }
        
        if (debug) {
        	SLog.d(TAG, "encryptMessage:body=" + Base64.encode(body));
        	SLog.d(TAG, "encryptMessage:bodyHash=" + Base64.encode(bodyHash));
        }
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:rsaEncrypt(bodyHash) time=" + clock.elapsedTime());
		
        if (bodySign == null) {
        	if (debug)
        		SLog.e(TAG, "encryptMessage:bodySign is null");
			result.error = Errors.MSG_BODY_RSA_SIGN_FAILED;
        	return result;
        }
        
        if (debug) {
        	SLog.d(TAG, "encryptMessage:bodySign=" + Base64.encode(bodySign));
        }
		...
		if (clock != null)
			SLog.d(TAG, "encryptMessage:aesEncrypt(body) time=" + clock.elapsedTime());
		
        if (encryptedData == null) {
        	if (debug)
        		SLog.e(TAG, "encryptMessage:encryptedData is null");
        ...
        if (debug) {
        	SLog.d(TAG, "encryptMessage:plainData=" + Base64.encode(bb.array()));
        	SLog.d(TAG, "encryptMessage:aesEncryptedData=" + Base64.encode(encryptedData));
        	SLog.d(TAG, "encryptMessage:iv=" + Base64.encode(iv));
        }
        ...
        if (debug) {
        	SLog.d(TAG, "encryptMessage:result.encKeyId=" + result.encKeyId);
        }
        
		if (clock != null)
			SLog.d(TAG, "encryptMessage:result buffer fill time=" + clock.elapsedTime());
	...
	public static MessageCryptResult decryptMessage(String privKeyS, long privKeyId, byte[] msgBlock, MessageCryptResolver resolver, IRealClock clock) {
		...
		if (debug) {
			SLog.d(TAG, "decryptMessage:privKey" + privKeyS);
			SLog.d(TAG, "decryptMessage:privKeyId" + privKeyId);
			SLog.d(TAG, "decryptMessage:msgBlock=" + Base64.encode(msgBlock));
		}
		
		if (msgBlock.length <= 12) {
			if (debug)
				SLog.e(TAG, "decryptMessage:msgBlock.length =" + msgBlock.length + " is invalid");
		...
		if ((dataLength == 0) || (keyLength == 0) || (ivLength == 0)) {
			if (debug)
				SLog.e(TAG, "decryptMessage:dataLength=" + dataLength + " keyLength=" + keyLength + " ivLength=" + ivLength);
		...
		if ((dataLength + keyLength + ivLength + 12) != msgBlock.length) {
			if (debug)
				SLog.e(TAG, "decryptMessage:dataLength=" + dataLength + " keyLength=" + keyLength + " ivLength=" + ivLength + " msgBlock.length=" + msgBlock.length);
		...
		if (debug) {
			SLog.d(TAG, "decryptMessage:encryptedData=" + Base64.encode(encryptedData));
			SLog.d(TAG, "decryptMessage:encryptedAesKey=" + Base64.encode(encryptedAesKey));
			SLog.d(TAG, "decryptMessage:iv=" + Base64.encode(iv));
		}
		
		byte[] aesKey = rsaDecrypt(privKey, encryptedAesKey);
		if (aesKey == null) {
			if (debug)
				SLog.e(TAG, "decryptMessage:aesKey=null");
		...
		if (debug) {
			SLog.d(TAG, "decryptMessage:aesKey.size=" + aesKey.length + " aesKey=" + Base64.encode(aesKey));
		}
...
//		System.out.println("aes_key=" + new String(aesKey));
		...
		if (plainData == null) {
			if (debug)
				SLog.e(TAG, "decryptMessage:plainData=null");
			result.error = Errors.AES_MSG_DECRYPT_FAILED;
			return result;
		}
		
		if (debug) {
			SLog.d(TAG, "decryptMessage:plainData=" + Base64.encode(plainData));
		}
		
		if (plainData.length <= 8) {
			if (debug)
				SLog.e(TAG, "decryptMessage:plainData.length=" + plainData.length);
		...
		if ((bodySignLength == 0) || (bodyLength == 0)) {
			if (debug)
				SLog.e(TAG, "decryptMessage:bodySignLength=" + bodySignLength + " bodyLength=" + bodyLength);
		...
		if ((bodyLength + bodySignLength + 8) != plainData.length) {
			if (debug)
				SLog.e(TAG, "decryptMessage:bodyLength=" + bodySignLength + " bodyLength=" + bodyLength + " plainData.length=" + plainData.length);
		...
		bb.get(body);
		if (debug) {
			SLog.d(TAG, "decryptMessage:body=" + Base64.encode(body));
			SLog.d(TAG, "decryptMessage:bodySign=" + Base64.encode(bodySign));
		}
		
		byte[] bodyHash = sha256(body);
		if (bodyHash == null) {
			if (debug)
				SLog.e(TAG, "decryptMessage:bodyHash=null");
			result.error = Errors.CALC_MSG_BODY_HASH_FAILED;
			return result;
		}
		
		if (debug) {
			SLog.d(TAG, "decryptMessage:bodyHash=" + Base64.encode(bodyHash));
		}
		
		Message msg = new Message();
		if (!msg.parseBytes(body)) {
			if (debug)
				SLog.e(TAG, "decryptMessage:msg not unpacked");
			result.error = Errors.MSG_UNPACK_FAILED;
			return result;
		}
		
		if (debug) {
			SLog.d(TAG, "decryptMessage:msg.id=" + msg.id);
			SLog.d(TAG, "decryptMessage:msg.encKeyId=" + msg.encKeyId);
			SLog.d(TAG, "decryptMessage:msg.signKeyId=" + msg.signKeyId);
			SLog.d(TAG, "decryptMessage:msg.from=" + msg.from);
			SLog.d(TAG, "decryptMessage:msg.peer=" + msg.peer);	
		}
		
		if (privKeyId != msg.encKeyId) {
			if (debug)
				SLog.e(TAG, "decryptMessage:msg enc key id is wrong");
		...
		if (queryResult.error != Errors.SUCCESS) {
			if (debug)
				SLog.e(TAG, "decryptMessage:getPubKeyById error=" + queryResult.error);
			result.error = queryResult.error;
			return result;			
		}
		...
		if (debug) {
			SLog.d(TAG, "decryptMessage:signKey=" + queryResult.publicKey);
			SLog.d(TAG, "decryptMessage:bodySign=" + Base64.encode(bodySign));
			SLog.d(TAG, "decryptMessage:plainBodyHash=" + Base64.encode(plainBodyHash));
			SLog.d(TAG, "decryptMessage:bodyHash=" + Base64.encode(bodyHash));
		}
		
		if (!Arrays.equals(plainBodyHash, bodyHash)) {
			if (debug)
				SLog.e(TAG, "decryptMessage:plainBodyHash != bodyHash");
	...
	public static void main(String[] args) {
		Security.addProvider(new org.bouncycastle.jce.provider.BouncyCastleProvider());
		SLog.start(false, "c:\\log.txt", null);
