...
public class Message implements IBytesDumpable {
	...
	public boolean parseBytes(byte[] data) {
		if (data.length < getHeaderLength()) {
			SLog.e(TAG, "invalid data size data.length=" + data.length);
			return false;
		}
		...
		if (msgSign != SIGN) {
			SLog.e(TAG, "invalid msg sign");
			return false;
		}
			
		if ((bytesLength < 0) || (mimeTypeLength < 0) || (displayNameLength < 0)) {
			SLog.e(TAG, "invalid bytesLength=" + bytesLength + " mimeTypeLength=" + mimeTypeLength
					+ " displayNameLength=" + displayNameLength);
			return false;
		}
		
		if ((bytesLength + mimeTypeLength + displayNameLength + getHeaderLength()) != data.length) {
			SLog.e(TAG, "invalid msg bytes length=" + bytesLength + " data.length=" + data.length);
			return false;
		}
